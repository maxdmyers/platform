	...
	<div class="item">
		...
		<section class="clearfix item-details">
			...
			<div class="well">
				...
				<?php echo Form::text('item_fields[{{id}}][name]', '{{name}}', array('id' => 'menu-items-{{id}}-name', 'placeholder' => Lang::line('menus::menus.general.name'))); ?>

				<?php echo Form::label('menu-items-{{id}}-slug', Lang::line('menus::menus.general.slug')); ?>
				<?php echo Form::text('item_fields[{{id}}][slug]', '{{slug}}', array('id' => 'menu-items-{{id}}-slug', 'placeholder' => Lang::line('menus::menus.general.slug'))); ?>

				<?php echo Form::label('menu-items-{{id}}-uri', Lang::line('menus::menus.general.uri')); ?>
				<?php echo Form::text('item_fields[{{id}}][uri]', '{{uri}}', array('id' => 'menu-items-{{id}}-uri', 'placeholder' => Lang::line('menus::menus.general.uri'), 'class' => 'menu-item-uri')); ?>

				<label class="checkbox">
					<input type="checkbox" name="item_fields[{{id}}][secure]" value="1" {{secure}} id="menu-items-{{id}}-secure" class="menu-item-secure">
